...
<meta name="keywords" content="{{$product->meta_keywords_en}}">


<meta property="og:url" content="{{ route('frontend.detailsbn',['category'=>$product->category->category_slug_en,'slug'=>$product->product_slug_en]) }}">



<meta name="description" content="{{$product->short_description_en}} ">
<meta property="og:type" content="article">
<meta property="og:title" content="{{$product->product_name_en}}">

<meta property="og:image"  itemprop="image"  content="{{ url($product->image) }}">
<meta name="twitter:image" content="{{ url($product->image) }}">

<meta property="og:description" content="{{$product->short_description_en}} ">

@endsection 
@else
@section('meta')
<meta name="description" content="{{$product->meta_description_bn}} ">
<meta name="keywords" content="{{$product->meta_keywords_bn}}">


<meta property="og:url" content="{{ route('frontend.detailsbn',['category'=>$product->category->category_slug_bn,'slug'=>$product->product_slug_bn]) }}">



<meta name="description" content="{{$product->short_description_bn}} ">
<meta property="og:type" content="article">
<meta property="og:title" content="{{$product->product_name_bn}}">

<meta property="og:image"  itemprop="image"  content="{{ url($product->image) }}">
<meta name="twitter:image" content="{{ url($product->image) }}">

<meta property="og:description" content="{{$product->short_description_bn}} ">
...
{{ (session()->get('language') === 'english') ? $product->product_name_en ." | ". $subcategory->subcategory_name_en : $product->product_name_bn ." | ". $subcategory->subcategory_name_bn}} 
        ...
        <div class="container">
          <div class="row al_center">
            <div class="col">
              ...
              @else
                <nav class="sp-breadcrumb">
                  <a href="{{ route('frontend.home') }}">الرئيسية</a><i class="facl facl-angle-right"></i><a
              ...
              <div class="col-md-6 col-12 product-infors pr_sticky_su">
                <div class="theiaStickySidebar">
                  <div class="kalles-section-pr_summary kalles-section summary entry-summary mt__30">
                    ...
                    <h2 class="product_title entry-title fs__16" style="color: #e91e63;">
                    ...
                    <div class="flex wrap fl_between al_center price-review">
                      <p class="price_range"
                        ...
                        @if (isset($product->discount))
                          <del id="price">L.E {{ $product->price }}</del>
                          <ins
                            id="new_price">L.E {{ $product->price - ($product->discount * $product->price) / 100 }}</ins>
                        @else
                          <ins id="price">L.E {{ $product->price }}</ins>
                        @endif
                      ...
                      <p>
                        ...
                        @endif
                                ...
                                @if (session()->get('language') === 'english')
                                  ...
                                  <a href="https://api.whatsapp.com/send?phone=+02{{ $subcategory->whatsapp_num }}&text={{ url('/bn/details/'.$category->category_slug_bn) }}/{{ $product->product_slug_bn }}  PRD ممكن استفسر عن المنتج؟" target="_blank"
                                          data-time="6000"
                                          data-ani="shake"
                                          class=" button  animated "
                                        >
                                  <img src="{{ url('images/wats.png') }}" width="70px" height="50px"> 
                                  
                                      @if ($category->meta_keywords_en=="coworking")
                                      <span class="txt_add ">ِAsk About Service</span>
                                      @else
                                      <span class="txt_add ">Ordrr via whatsapp</span>
                                      @endif
...
<br>
    ...
    color: #4caf50;
                                  ...
                                  href="https://api.whatsapp.com/send?phone=+02{{ $subcategory->whatsapp_num }}&text={{ url('/bn/details/'.$category->category_slug_bn) }}/{{ $product->product_slug_bn }}  PRD ممكن استفسر عن المنتج؟" target="_blank" >
                                  
                                   {{ $subcategory->whatsapp_num }} 
                            </a>
                              ...
                              <a href="https://api.whatsapp.com/send?phone=+02{{ $subcategory->whatsapp_num }}&text={{ url('/en/details/'.$category->category_slug_en) }}/{{ $product->product_slug_en }}  PRD ممكن استفسر عن المنتج؟" target="_blank"
                                data-time="6000"
                                data-ani="shake"
                                class=" button  animated "
                              >
                        <img src="{{ url('images/wats.png') }}" width="70px" height="50px"> 
                        @if ($category->icon=="coworking")
                        <span class="txt_add ">ِاسأل عن الخدمة</span>
                        @else
                        <span class="txt_add ">اطلب عبر واتس اب</span>
                        @endif
                        </a>
<br>
                        <a 
...
color: #4caf50;
                        ...
                        href="https://api.whatsapp.com/send?phone=+02{{ $subcategory->whatsapp_num }}&text={{ url('/en/details/'.$category->category_slug_en) }}/{{ $product->product_slug_en }}  PRD ممكن استفسر عن المنتج؟" target="_blank" >
...
<br>
                    ...
                    <div class="product_meta">
                      ...
                      @else
                        <span class="posted_in"><span class="cb">التصنيف:</span> <a href="{{ route('categoryproductsbn', $product->category->category_slug_bn) }}"
                            class="cg"
        ...
        <div class="kalles-medical__deal-section container">
          <div class="medizin_laypout">
            ...
            <div class="products nt_products_holder row fl_center row_pr_1 js_carousel nt_slider nt_cover ratio1_1 position_8 space_ prev_next_3 btn_owl_1 dot_owl_1 dot_color_1 btn_vi_2 equal_nt" data-flickity='{"imagesLoaded": 0,"adaptiveHeight": 0, "contain": 1, "groupCells": "100%", "dragThreshold" : 5, "cellAlign": "left","wrapAround": true,"prevNextButtons": true,"percentPosition": 1,"pageDots": false, "autoPlay" : 0, "pauseAutoPlayOnHover" : true, "rightToLeft": false }'>
              ...
              @else <div class="col-lg-15 col-md-3 col-6 pr_animated done mt__30 pr_grid_item product nt_pr desgin__2 tc">
                <div class="product-inner pr">
                  <div class="product-image pr oh lazyload">
                    <span class="tc nt_labels pa pe_none cw"><span class="nt_label new {{ isset($product->discount) ? 'bg-danger' : '' }}">{{ isset($product->discount) ? '- '.$product->discount . ' %' : 'New' }}</span></span>
                    ...
                    <div class="hover_button op__0 tc pa flex column ts__03">
                      ...
                      <a  class="pr pr_atc cd br__40 bgw tc dib js__qs cb chp ttip_nt tooltip_top_left  productView" href="#"  product_id={{ $product->id }}><span class="tt_txt">اضف الي العربة</span><i class="iccl iccl-cart"></i><span>اضف الي العربة</span></a>
                    </div>
                  </div>
                  <div class="product-info mt__15">
                    ...
                    @if (isset($product->discount))
                    <p class="price_range" id="price_qv">
                      <del> L.E {{ $product->price }}</del>
                      <ins> L.E {{ round($product->price - (($product->discount * $product->price) / 100)) }}</ins>
                    </p>
                    @else
                    <span class="price dib mb__5"> ৳ {{ $product->price }} <span class="text-danger"></span></span>
                    @endif
