...
class ProductController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $getname_en=Product::where('product_slug_en',Str::slug($request->product_name_en))->count();
        if($getname_en>0){
            $slugen=Str::slug($request->product_name_en).rand(0,9999);
        }else{
            $slugen=Str::slug($request->product_name_en);
        }

        
        $getname_bn=Product::where('product_slug_bn',$this->make_slug($request->product_name_bn))->count();
        if($getname_bn>0){
            $slugbn=$this->make_slug($request->product_name_bn).rand(0,9999);
        }else{
            $slugbn=$this->make_slug($request->product_name_bn);
        }
        ...
        $product   = Product::create([
            ...
            'product_slug_en'               => $slugen,
            'product_slug_bn'               => $slugbn,
    ...
    protected function uploadeImage($request)
    {
        ...
        $get_imageName  =  date('mdYHis') . uniqid() . str_replace($file->getClientOriginalName(),' ','_').'.jpg';
